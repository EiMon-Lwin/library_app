import 'dart:ui';

const double kSP3x=3;
const double kSP5x=5;
const double kSP8x=8;
const double kSP10x=10;
const double kSP15x=15;
const double kSP20x=20;
const double kSP40x=40;
const double kSP50x=50;
const double kSP55x=55;
const double kSP30x=30;
const double kSP8ox=80;

const double kTextFieldSizedWidth380x=380;
const double kTextFieldSizedHeight70x=70;
const double kBookShelfHeight570x=570;
const double kOneShelfHeight360x=360;
const double kBookImageItemViewHeight300x=300;
const double kBookImageItemViewWidth150x=150;
const double kShowBottomSheetWidgetHeight230x=230;
const double kImageHeight230x=230;
const double kTitleHeight60x=60;
const double kCircularImageWidth60x=60;
const double kCircularImageHeight45x=45;
const double kCircularImageRadius45x=45;
const double kRadiusCircular15x=15;
const double kSearchTextFieldWidth290x=290;

const double kDetailsImageWidth200x=200;
const double kDetailsImageHeight300x=300;
const double kDetailsAuthorNameBoxHeight50x=50;

const double kBottomSheetImageWidth50x=50;
const double kBottomSheetImageHeight80x=80;

const double kImageBorderCircular8x=8;

const double kBookTitleWidth250x=250;
const double kBookTitleHeight65x=65;

/// Library Page
const double kFavoriteBookSessionHeight355x=355;
const double kFavoriteBookItemViewHeight320x=320;
const double kCachedNetworkImageHeight250x=250;
const double kCachedNetworkImageWidth150x=150;
const double kBorderRadiusCircular8x=8.0;
const double kTitleWidth150x=150;

///FontWeight
const FontWeight kFontWeightBold=FontWeight.bold;



///IconSize
const double kIconSize15x=15;